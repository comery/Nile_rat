...
### Code usage and config

you can download all scripts using

```git clone https://github.com/comery/Nile_rat.git```

and then set the ```bin```path for below analyzing

```bin=/xxx/xxx/Nile_rat/bin```



### Manually improve the assembly

Anchor more scaffolds onto chromosomes and prepare data to whole-genome alignment and SV identification.
...
```
> Note: you can copy the circos.conf form [here](https://github.com/comery/Data-Visulization/blob/master/Genome_synteny/example/circos.conf)

show cirocs
![circos1](./images/circos.raw.png) <center>Figure 1. maternal vs. paternal alignment</center>

manually adjust the order, ```vi karyotype.txt```
draw again using ```circos -conf circos.conf```

show circos again
![circos2](./images/circos.adjust_order.png) <center>Figure 2. maternal vs. paternal alignment with adjusted orders</center>


#### Trim alignments to generate continuous and confident blocks
![find miss scaffolds](./images/scaffold_anchor_chr.png) <center>Figure 3. preview of missing scaffolds</center>
...
![how to trim alignments](./images/trim_alignments.png) <center>Figure 4. how to trim alignments</center>
...
```
![alignment](./images/chr4.alignment.png) <center>Figure 5. simple alignment, show chr4 as a case</center>

But other cases are slightly complex, like ```chr1,chr2,chr3,chr6,chr16,chr17,chr19,chr21,chr28,chr29,chr31```;

![alignment](./images/chr1.alignment.png) <center>Figure 6. complex alignment, show chr1 as a case</center>
...
![alignment](./images/chr6-alignment.png) <center>Figure 7. alignment of chr6</center>

![alignment](./images/chr6-checkpoint1.png) <center>Figure 8. check left breakpoint on paternal #1 </center>

![alignment](./images/chr6-checkpoint2.png) <center>Figure 9. check right breakpoint on paternal #1 </center>
...
#### Rethinking of this step

Recently, I find a tookit - [RagTag](https://github.com/malonge/RagTag) can do the similar thing like I did above. if you have interest in it, you can play with it.
...
![image](./images/variant_affect_gene.png)
...
#### Date usage
I downloaded the rat RefSeq genome (GCF_015227675.2_mRatBN7.2_genomic.fna.gz), and the mouse RefSeq genome (GCF_000001635.27_GRCm39_genomic.fna.gz) and others you need.
...
To achieve this goal, I generated a dataset of 20,426 genes of human by selecting the longest transcript as the representative transcript. Then annotated rat, mouse, and Nile rat genomes using [exonerate](https://www.ebi.ac.uk/about/vertebrate-genomics/software/exonerate). 
...
First, each included gene had to map to the human sequence alignments including at least 70% of its coding sequence (CDS). Second, frameshift indels in CDSs were prohibited. Third, CDSs with premature stop codons were excluded. In addition, final alignments shorter than 150 bp were discarded. 
...
# collect all significant genes together
...
# generate images that contain phylogenetic tree and local sequence alignment
...
# some genes may no have sites, if will give error report in draw.log
```
...
#### reference
[1] https://www.protocols.io/view/introduction-to-calculating-dn-ds-ratios-with-code-qhvdt66?version_warning=no&step=4